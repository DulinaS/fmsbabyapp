...
class _GrowthMilestonePageState extends State<GrowthMilestonePage> {
  ...
  Future<void> _loadChildData() async {
    ...
    try {
      ...
      _childService.getDailyWeights(widget.childId).listen((snapshot) async {
        print(
          "DEBUG: Firestore listener triggered! Document count: ${snapshot.docs.length}",
        );
        ...
        if (mounted) {
          ...
          print("DEBUG: Processing ${sortedDocs.length} weight documents");
          ...
          for (var doc in sortedDocs) {
            ...
            final dateKey = dateToKey(date);
            final weight = data['weight'] as double;
            final dayNumber = data['dayNumber'] as int? ?? dayCounter;

            print(
              "DEBUG: Processing doc - Date: $date, Weight: ${weight}g, DayNumber: $dayNumber",
            );
            ...
            // Update date to day mapping
            newDateToDayMap[dateKey] = dayNumber;
            newDayToDateMap[dayNumber] = date;

            // Always update weight data for this day
            ...
            // Update day counter for next iteration
            if (dayNumber >= dayCounter) {
              dayCounter = dayNumber + 1;
            }
          }

          // Debug prints before updating state
          print("DEBUG: Final newWeightData keys: ${newWeightData.keys}");
          for (var entry in newWeightData.entries) {
            print("DEBUG: Final Day ${entry.key} -> Weight: ${entry.value}g");
          }
          ...
          if (mounted) {
            ...
            print(
              "DEBUG: State updated! Current weightData keys: ${weightData.keys}",
            );
  ...
  /* Widget _buildGrowthChart() {
    ...
    // Calculate chart width based on data range
    // More space between points = better readability
    double chartWidth = (maxXAdjusted - minXAdjusted) * 8.0; // 8 pixels per day
    chartWidth = chartWidth < 800 ? 800 : chartWidth; // Minimum width of 800
                            ...
                            LineChartData(
                              gridData: FlGridData(
                                ...
                                horizontalInterval:
                                    1.0, // Always in kg with proper spacing
                                verticalInterval:
                                    15, // Every 15 days instead of 30
                                getDrawingHorizontalLine: (value) {
                                  return FlLine(
                                    color: Colors.grey.shade300,
                                    strokeWidth: 1,
                              ...
                              titlesData: FlTitlesData(
                                ...
                                bottomTitles: AxisTitles(
                                  sideTitles: SideTitles(
                                    ...
                                    interval:
                                        15, // Show every 15 days for better spacing
                                  ),
                                ),
                                leftTitles: AxisTitles(
                                  sideTitles: SideTitles(
  ...
  //After all done below is most corrected
  ...
  /* Widget _buildGrowthChart() {
    ...
    return Container(
      width: double.infinity,
      height: 520, // Increased height significantly
      decoration: ShapeDecoration(
        color: Colors.white,
        shape: RoundedRectangleBorder(
          side: BorderSide(
                  ...
                  _isLoadingStandards && spots.isEmpty
                      ...
                      : Padding(
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: Container(
                            width: chartWidth,
                            height: 320, // Set explicit height for chart area
                            child: LineChart(
                              LineChartData(
                                gridData: FlGridData(
                                  show: true,
                                ...
                                lineTouchData: LineTouchData(
                                  touchTooltipData: LineTouchTooltipData(
                                    ...
                                    tooltipMargin:
                                        16, // Add margin to prevent cutoff
